...
def _save() -> None:
    ...
    get_graph().serialize(destination=path, format="turtle")
...
def kg_describe(entity: str) -> str:
    ...
    incoming = [f"  {_qname(s)} {_qname(p)} {_qname(node)}" for s, p in graph.subject_predicates(node)]
...
def kg_sparql(query: str, limit: int = 50) -> str:
    ...
    body = "\n".join(" | ".join(_qname(v) if v is not None else "" for v in row) for row in rows)
    return f"{header}\n{body}"
